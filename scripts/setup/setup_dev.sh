...
# Instalar Docker
print_status "2" "Instalando Docker..." "Em andamento"
if curl -fsSL https://get.docker.com -o get-docker.sh && sh get-docker.sh; then
    print_status "2" "Instalando Docker..." "Sucesso"
else
    print_status "2" "Instalando Docker..." "Falha"
    exit 1
fi

# Instalar Docker Compose
print_status "3" "Instalando Docker Compose..." "Em andamento"
if sudo curl -L "https://github.com/docker/compose/releases/download/1.29.2/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose && sudo chmod +x /usr/local/bin/docker-compose; then
    print_status "3" "Instalando Docker Compose..." "Sucesso"
else
    print_status "3" "Instalando Docker Compose..." "Falha"
    exit 1
fi

# Instalar Portainer
print_status "4" "Instalando Portainer..." "Em andamento"
if sudo docker volume create portainer_data && sudo docker run -d -p 9000:9000 --name portainer --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce; then
    print_status "4" "Instalando Portainer..." "Sucesso"
else
    print_status "4" "Instalando Portainer..." "Falha"
    exit 1
fi

# Instalar Ansible
print_status "5" "Instalando Ansible..." "Em andamento"
if sudo apt-get install -y ansible; then
    print_status "5" "Instalando Ansible..." "Sucesso"
else
    print_status "5" "Instalando Ansible..." "Falha"
    exit 1
fi

# Instalar Python
print_status "6" "Instalando Python..." "Em andamento"
if sudo apt-get install -y python3 python3-pip; then
    print_status "6" "Instalando Python..." "Sucesso"
else
    print_status "6" "Instalando Python..." "Falha"
    exit 1
fi

# Instalar Go
print_status "7" "Instalando Go..." "Em andamento"
if wget https://golang.org/dl/go1.16.5.linux-amd64.tar.gz && sudo tar -C /usr/local -xzf go1.16.5.linux-amd64.tar.gz; then
    ...
    source ~/.profile
    print_status "7" "Instalando Go..." "Sucesso"
else
    print_status "7" "Instalando Go..." "Falha"
    exit 1
fi

# Limpeza
print_status "8" "Limpando arquivos temporários..." "Em andamento"
if rm -f get-docker.sh go1.16.5.linux-amd64.tar.gz; then
    print_status "8" "Limpando arquivos temporários..." "Sucesso"
else
    print_status "8" "Limpando arquivos temporários..." "Falha"
    exit 1
fi

print_status "9" "Setup dev completo!" ""
